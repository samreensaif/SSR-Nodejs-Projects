...
//---------------------------------------Rules end here--------------------------------------------------------
let score = 0;
let decScore = 1000; // get score 10 after each correct answer
let totalQuestions = 0; // number of questions whose answers are correct
let isfifty50Used = false; // fifty-fifty life-line equals 0 means not used yet
let pname;
const question = {
    question1: "Canberra",
...
function fifty50Option() {
    let correctAns = question.question1;
    prompt1.choices.filter((choice) => choice === correctAns);
...
async function playerName() {
    ...
    while (!isValidName) {
        ...
        if (player.pname.trim() === "") {
            ...
            isValidName = true;
            let nameTC = player.pname.split(" ").map((a) => a.charAt(0).toUpperCase() + a.slice(1).toLowerCase()).join(" ");
            return nameTC;
        }
    }
}
